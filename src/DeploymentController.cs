...
static class DeploymentController
{
	...
	public static void HandleDeploymentInput()
	{
		...
		if (SwinGame.MouseClicked(MouseButton.LeftButton)) {
			...
			if (GameController.HumanPlayer.ReadyToDeploy & UtilityFunctions.IsMouseInRectangle (PLAY_BUTTON_LEFT, TOP_BUTTONS_TOP, PLAY_BUTTON_WIDTH, TOP_BUTTONS_HEIGHT)) {
				...
				GameController.HumanPlayer.RandomizeDeployment ();
			} else if (UtilityFunctions.IsMouseInRectangle (700, 550, 51, 46)) {
	...
	private static void DoDeployClick()
	{
		...
		int col = 0;
		/// <summary>
		/// Solved : Fix the position of the ship when clicking outside the grid and fix position the ship on the mouse pointer.
		/// When the user clicked somewhere on the screen nothing will happen. If the click is in the grid it deploys to the selected location.
		/// </summary>
		//Changed the row = Convert.ToInt32(Math.Floor((mouse.y)/... to current state, Willone (7/4/2017).
